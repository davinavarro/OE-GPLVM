            ...
            for param in param_grid:
                try:
                    if self.model_name == "IForest":
                        ...
                        model = self.model_dict[self.model_name](kernel=param).fit(
                            X_train
                        )

                    elif self.model_name == "ABOD":
                        model = self.model_dict[self.model_name](n_neighbors=param).fit(
                            X_train
                        )

                    elif self.model_name == "CBLOF":
                        model = self.model_dict[self.model_name](n_clusters=param).fit(
                            X_train
                        )
                        ...
                        model = self.model_dict[self.model_name](n_bins=param).fit(
                            X_train
                        )
                        ...
                        model = self.model_dict[self.model_name](dis_measure=param).fit(
                            X_train
                        )
                        ...
                        model = self.model_dict[self.model_name](alpha=param).fit(
                            X_train
                        )

                    elif self.model_name == "LSCP":
                        model = self.model_dict[self.model_name](
                            detector_list=[LOF(), LOF()], n_bins=param
                        ).fit(X_train)
                        ...
                        model = self.model_dict[self.model_name](perplexity=param).fit(
                            X_train
                        )

                    elif self.model_name == "SOGAAL":
                        model = self.model_dict[self.model_name](stop_epochs=param).fit(
                            X_train
                        )
                        ...
                        model = self.model_dict[self.model_name](epochs=param).fit(
                            X_train
                        )
