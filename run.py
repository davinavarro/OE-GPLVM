...
class RunPipeline:
    def __init__(
        ...
        self.data_generator = DataGenerator(
            generate_duplicates=self.generate_duplicates,
    ...
    def dataset_filter(self):
        ...
        dataset_list_org = [
            ...
            for _ in os.listdir("datasets/Classical")
            if os.path.splitext(_)[1] == ".npz"
        ]  # classical AD datasets
        dataset_list_org.extend(
            [
                os.path.splitext(_)[0]
        ...
        dataset_list_org.extend(
            [
                os.path.splitext(_)[0]
                for _ in os.listdir("datasets/NLP_by_BERT")
                if os.path.splitext(_)[1] == ".npz"
            ]
        )  # NLP datasets
        ...
        for dataset in dataset_list_org:
            ...
            for seed in self.seed_list:
                ...
                if (
                    not self.generate_duplicates
                    and len(data["y_train"]) + len(data["y_test"])
    ...
    def model_fit(self):
        try:
            ...
            if self.model_name in ["DevNet", "FEAWAD", "REPEN"]:
                self.clf = self.clf(
                    seed=self.seed, model_name=self.model_name, save_suffix=self.suffix
                )
            else:
            ...
            print(
                f"Error in model initialization. Model:{self.model_name}, Error: {error}"
            )
            pass
        ...
        try:
            ...
            if self.model_name == "DAGMM":
                score_test = self.clf.predict_score(
            ...
            # performance
            result = self.utils.metric(
                y_true=self.data["y_test"], y_score=score_test, pos_label=1
            )

            K.clear_session()
            print(
                f"Model: {self.model_name}, AUC-ROC: {result['aucroc']}, AUC-PR: {result['aucpr']}"
            )
    ...
    def run(self):
            ...
            if self.noise_type is not None:
                experiment_params = list(
                    product(
                        dataset_list,
                        self.nla_list,
                        self.noise_params_list,
                        self.seed_list,
                    )
                )
            else:
                experiment_params = list(
                    product(dataset_list, self.nla_list, self.seed_list)
                )
        else:
            if self.noise_type is not None:
                experiment_params = list(
                ...
                experiment_params = list(
                    product(dataset_list, self.rla_list, self.seed_list)
                )
        ...
        df_AUCROC = pd.DataFrame(
            data=None, index=experiment_params, columns=list(self.model_dict.keys())
        )
        df_AUCPR = pd.DataFrame(
            data=None, index=experiment_params, columns=list(self.model_dict.keys())
        )
        df_time_fit = pd.DataFrame(
            data=None, index=experiment_params, columns=list(self.model_dict.keys())
        )
        ...
        for i, params in tqdm(enumerate(experiment_params)):
            ...
            for model_name in tqdm(self.model_dict.keys()):
                ...
                df_AUCROC.to_csv(
                    os.path.join(
                        os.getcwd(), "result", "AUCROC_" + self.suffix + ".csv"
                    ),
                    index=True,
                )
                df_AUCPR.to_csv(
                    os.path.join(
                        os.getcwd(), "result", "AUCPR_" + self.suffix + ".csv"
                    ),
                    index=True,
                )
                df_time_fit.to_csv(
                    os.path.join(
                        os.getcwd(), "result", "Time(fit)_" + self.suffix + ".csv"
                    ),
                    index=True,
                )
                df_time_inference.to_csv(
                    os.path.join(
                        os.getcwd(), "result", "Time(inference)_" + self.suffix + ".csv"
                    ),
                    index=True,
                )
