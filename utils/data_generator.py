...
class DataGenerator:
        ...
        if noise_ratio == 0.0:
            ...
            if noise_dim > 0:
                ...
                for i in range(noise_dim):
                    ...
                    X_noise.append(
                        np.random.uniform(X_min, X_max, size=(X.shape[0], 1))
                    )
    ...
    def add_label_contamination(self, X, y, noise_ratio: float):
        if noise_ratio == 0.0:
            ...
            idx_flips = np.random.choice(
                np.arange(len(y)), int(len(y) * noise_ratio), replace=False
            )
    ...
    def generator(
        ...
        if realistic_synthetic_mode is not None:
            ...
            if realistic_synthetic_mode == "dependency":
                ...
                try:
                    ...
                    np.savez_compressed(
                        os.path.join("datasets", "synthetic", filepath), X=X, y=y
                    )
                    pass
        ...
        if noise_type == "duplicated_anomalies":
            X_train, y_train = self.add_duplicated_anomalies(
                X_train, y_train, duplicate_times=duplicate_times
            )
            X_test, y_test = self.add_duplicated_anomalies(
                X_test, y_test, duplicate_times=duplicate_times
            )

        # notice that label contamination can only be added in the training set
        elif noise_type == "label_contamination":
            X_train, y_train = self.add_label_contamination(
